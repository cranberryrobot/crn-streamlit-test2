...
def barchart():
    ...
    def from_data_file():
        ...
        long_lat_pairs = pd.Series(df['longitude']) + pd.Series(df['latitude'])
        st.write(long_lat_pairs)
        df['police_force.api_url'] = str("https://data.police.uk/api/locate-neighbourhood?q="+df['longitude']+","+df['latitude'])

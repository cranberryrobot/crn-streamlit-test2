...
def barchart():
    ...
    def from_data_file():
        ...
        df['police_force.api_url'] = df.agg(lambda x: f"https://data.police.uk/api/locate-neighbourhood?q={x['latitude']},{x['longitude']}", axis=0)
        st.write(df.columns.tolist())
        df['police_force'] = df.apply(lambda x: pd.read_json(x['police_force.api_url']))
